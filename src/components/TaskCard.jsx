...
const TaskCard = ({ task, onUpdate, onDelete }) => {
  ...
  return (
    <>
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 p-6 border border-gray-100 dark:border-gray-700 relative group h-full flex flex-col">
        <div className={`absolute left-0 top-4 bottom-4 w-1 rounded-r-full ${getPriorityColor(task.priority)}`}></div>
        
        <div className="flex justify-between items-start mb-3 pl-3">
          <div className="flex-1 pr-2">
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-1 leading-tight">{task.taskTitle}</h3>
            <p className="text-gray-500 dark:text-gray-400 text-xs line-clamp-2 min-h-[2.5em]">{task.taskDescription || "No description provided."}</p>
          </div>
          {/* Tombol Aksi */}
        ...
        <div className="pl-3 mt-auto space-y-2">
          <div className="flex flex-wrap gap-2 text-xs">
            <span className="px-2.5 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-md font-medium">
               {task.taskCategory}
            </span>
            <span className="px-2.5 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-md font-medium flex items-center">
               ⏰ {task.dueDate}
            </span>
          </div>
        </div>
